...
for n = 1:N
    ...
    while temp_z <= 100
        for j = 1:J
            reward_j = myPE_IE(truetheta, belieftheta, att, var_obs, vartheta, varlt, temp_z);
            %reward_j = myIE_PE(truetheta, belieftheta, att, var_obs, vartheta, varlt, temp_z);
            %reward_j = myIE_KG(truetheta, belieftheta, att, var_obs, vartheta, varlt, temp_z);
            %reward_j = myKG_IE(truetheta, belieftheta, att, var_obs, vartheta, varlt, temp_z);
            REWARD_explt(J*(n-1)+j) = reward_j;
        end
        z(temp_z) = z(temp_z)+mean(REWARD_explt);
    end
    n
end
